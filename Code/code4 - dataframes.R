# Data Frame (like an Excel spreadsheet)
    # make a list, conver to dataframe
    set.seed(1234)
    mylist <- list(
        col1 = 1:10,
...
# Combining dataframes
    ...
    # merge (aka "join")
    merge(df1, df2, by="aa")
    merge(df1, df2, by="aa", all.x=T)
    merge(df1, df2, by="aa", all.y=T)
    merge(df1, df2, by="aa", all=T)
...
# summarizing (collapsing / aggregating) a dataframe
    ...
    aggregate(df$ints, by=df[, c("lets", "caps")], mean)
    
    # aggregate -- args take a formula
    aggregate(ints ~ lets, data=df, mean)
    aggregate(ints ~ lets + caps, data=df, mean)
...
# reshaping a dataframe
    ...
    gather(data  = df_wide,     # specify the dataframe 
           ...
           -student_id)         # all the cols to reshape (or minus those NOT to rehape)
